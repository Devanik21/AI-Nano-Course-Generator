...
with st.sidebar:
    ...
    # Get defaults from URL query params to allow sharing
    query_params = st.query_params.to_dict()
    default_topic = query_params.get("topic", "Introduction to Machine Learning")
    default_duration_label = query_params.get("duration", "10 minutes")
    default_difficulty = query_params.get("difficulty", "Intermediate")
    default_learning_style = query_params.get("style", "Multimodal")
    default_content_format = query_params.get("format", "Comprehensive")
    default_prerequisites = query_params.get("prereqs", "")
    default_objectives = query_params.get("objectives", "").replace("|||", "\n")
    ...
    # Course Parameters
    topic = st.text_input("📚 Course Topic:", value=default_topic, help="Enter any topic you want to learn about")
    
    duration_keys = list(COURSE_DURATIONS.keys())
    duration_index = duration_keys.index(default_duration_label) if default_duration_label in duration_keys else 2
    duration_label = st.selectbox("⏱️ Course Duration:", duration_keys, index=duration_index)
    duration = COURSE_DURATIONS[duration_label]
    
    difficulty_keys = list(DIFFICULTY_LEVELS.keys())
    difficulty_index = difficulty_keys.index(default_difficulty) if default_difficulty in difficulty_keys else 1
    difficulty = st.selectbox("📊 Difficulty Level:", difficulty_keys, index=difficulty_index)
    
    learning_style_keys = list(LEARNING_STYLES.keys())
    learning_style_index = learning_style_keys.index(default_learning_style) if default_learning_style in learning_style_keys else 4
    learning_style = st.selectbox("🧠 Learning Style:", learning_style_keys, index=learning_style_index)
    
    content_format_keys = list(CONTENT_FORMATS.keys())
    content_format_index = content_format_keys.index(default_content_format) if default_content_format in content_format_keys else 0
    content_format = st.selectbox("📋 Content Format:", content_format_keys, index=content_format_index)
    
    with st.expander("🎯 Advanced Options"):
        prerequisites = st.text_area("Prerequisites:", value=default_prerequisites, placeholder="List any required background knowledge...")
    ...
    with col2:
        if st.button("📄 Export as DOCX"):
            with st.spinner("Generating DOCX..."):
                docx_content = export_course_content(st.session_state.course_content, format_type="docx")
                if docx_content:
                    st.download_button(
                        label="📥 Download DOCX",
                        data=docx_content,
                        file_name=f"{topic.replace(' ', '_')}_course.docx",
...
- Consider related courses for broader knowledge
    ...
    with col4:
        if st.button("🔗 Share Course"):
            ...
            params_to_share = {k: v for k, v in params_to_share.items() if v}
            st.query_params.update(**params_to_share)
            
            st.success("✅ URL updated! Copy the full link from your browser's address bar. 👆")
            st.info("Your course settings have been added to the URL. You can now copy it to share.")
            # Display the parameters in a read-only text area for clarity and easy copying
            import urllib.parse
            query_string = urllib.parse.urlencode(params_to_share)
            st.text_area("Shareable Parameters (added to URL)", f"?{query_string}", height=120, disabled=True, help="These parameters have been added to the URL in your browser's address bar.")
